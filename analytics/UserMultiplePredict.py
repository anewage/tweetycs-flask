...
def predict(inputFile, outputFile, parameterOut,type):
    if type == 's':
        with open('SVMUserOnly.pickle','rb') as f:
            load_model=pickle.load(f)
    if type =='r':
        with open('RFUserOnly.pickle','rb') as f:
            load_model=pickle.load(f)
    if type =='l':
        with open('KNNUserOnly.pickle','rb') as f:
            load_model=pickle.load(f)
    if type =='m':
        with open('MLPUserOnly.pickle','rb') as f:
            load_model=pickle.load(f)
...
if __name__ == '__main__':
    ...
    if classifier =='s':
        predict(inputFile,outputFile,'newuser_category','s')
    if classifier =='r':
        predict(inputFile,outputFile,'newuser_category','r')
    if classifier =='k':
        predict(inputFile,outputFile,'newuser_category','k')
    if classifier =='m':
        predict(inputFile,outputFile,'newuser_category','m')
