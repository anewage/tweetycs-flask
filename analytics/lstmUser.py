...
TRAIN_PROCESSED_FILE = 'CombinedTaggedTweets.csv'
...
def process_tweets(csv_file, test_file=True):
	...
	tweet = bDataFrame['user_description']
	senti = bDataFrame['user_category']
	total = len(tweet)
	for t in tweet:
...
if __name__ == '__main__':
	...
	tweetm = mDataFrame['user_description']
	...
	if train:
		...
		model.add(Dense(6, activation='softmax',use_bias=False)) # Dense=>全连接层,输出维度=3
		...
		mDataFrame['newUserCategory']=le.inverse_transform(results)
